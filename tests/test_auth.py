...
def test_register_user_success(client, auth_data):
    ...
    token = AuthToken.objects(_user=user.id, _token_type="email_verification").first()
    assert token is not None
    assert token.get_user().id == user.id
    assert token.type == "email_verification"
...
def test_resend_verification_success_with_existing_token(client, auth_data):
    ...
    user = User.objects(_email=auth_data['email']).first()
    tokens = AuthToken.objects(_user=user, _token_type="email_verification")
