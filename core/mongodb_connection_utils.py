...
def _rate_performance(time_seconds, is_atlas):
    ...
    if is_atlas:
        if time_seconds < 2: return "Excellent"
        elif time_seconds < 5: return "Good"
        elif time_seconds < 10: return "Fair"
        else: return "Poor"
    else:
        if time_seconds < 0.5: return "Excellent"
        elif time_seconds < 1: return "Good"
        elif time_seconds < 2: return "Fair"
        else: return "Poor"
...
def _rate_query_performance(time_seconds, is_atlas):
    ...
    if is_atlas:
        ...
        elif time_seconds < 3: return "Fair"
        else: return "Poor"
    else:
        if time_seconds < 0.1: return "Excellent"
        elif time_seconds < 0.3: return "Good"
        elif time_seconds < 0.5: return "Fair"
        else: return "Poor"
...
def print_connection_report(uri=None):
    ...
    if conn_result['success']:
        ...
        print(f"   Server Version: {conn_result['server_info'].get('version', 'Unknown')}")
        
        perf = conn_result['performance']
        print(f"\n⚡ Performance:")
        print(f"   Connection Time: {perf['connection_time']}s ({perf['connection_rating']})")
        ...
        if ops_result['success']:
            ...
            print(f"   Status: ❌ Operations failed")
            if ops_result['error']:
