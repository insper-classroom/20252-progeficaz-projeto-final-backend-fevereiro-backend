...
def register(data: dict) -> api_response:
    ...
    if not all([email, password]):
        return jsonify({"msg": "Campos obrigatórios: email, password"}), 400
    ...
    if not (
        ...
        return jsonify(
            {"msg": "Utilize seu email Insper (@al.insper.edu.br ou @insper.edu.br)"}
        ), 400
...
def login(data: dict) -> api_response:
    ...
    # Validando informações faltantes
    if not all([email, password]):
        return jsonify({"msg": "Campos obrigatórios: email, password"}), 401
